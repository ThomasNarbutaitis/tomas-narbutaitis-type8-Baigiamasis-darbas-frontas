...
function UpdateAnswer() {
  ...
  const getAnswerId = useParams();

  async function getAnswer() {
    const fetchResult = await myFetch(`${baseUrl}/answers/${getAnswerId.id}`);
    // console.log('fetchResult ===', fetchResult.question);
    setFormValues(fetchResult.answer);
    setQId(fetchResult.question_id);
  }
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      const fetchResult = await myFetchAuth(
        `${baseUrl}/answers/${getAnswerId.id}`,
        token,
        'PATCH',
        values
      );
      // console.log('fetchResult ===', fetchResult);
      if (fetchResult === 'answer updated') {
        setFeedback('Answer was updated successfully');
  ...
  function goBack() {
    history.replace(`/QuestionAnswer/${qId}`);
  }
  return (
    <Container>
      <Title>Update your answer Here</Title>
